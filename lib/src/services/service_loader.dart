...
class ServiceLoader {
  ...
  bool isLoading(dynamic ref, dynamic name) => (ref ?? MyServices.ref)?.watch(_loadings).isLoading(name) ?? false;

  void setLoading(dynamic name, bool status) => MyServices.ref.read(_loadings.notifier).setLoading(name, status);
}
